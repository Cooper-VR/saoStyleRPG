Shader "Custom/newCliffShaderText"
{
	Properties
	{
		[TerrainCompatible]
		...
		_Control("Control texture", 2D) = "white" {}

		_Splat0("Control texture", 2D) = "white" {}
		_Splat1("Control texture", 2D) = "white" {}
		_Splat2("Control texture", 2D) = "white" {}
		_Splat3("Control texture", 2D) = "white" {}

		_SmoothStepLow("SmoothStepLow", Range(0,2)) = 0
		_SmoothStepHigh("SmoothStepHigh", Range(0,2)) = 0.1
		...
		SubShader
		{
			...
			float _SmoothStepLow;
			float _SmoothStepHigh;
			...
			void surf(Input IN, inout SurfaceOutputStandard o)
			{
				...
				col = splatControl.r * tex2D(_Splat0, IN.uv_Control * _Splat0_ST.xy);
				...
				float threshold = smoothstep(_SmoothStepLow, _SmoothStepHigh, abs(dot(vec, _normalVector)));
