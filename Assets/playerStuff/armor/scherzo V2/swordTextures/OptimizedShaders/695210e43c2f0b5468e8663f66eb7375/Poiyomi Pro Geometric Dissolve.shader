Shader "Hidden/Locked/.poiyomi/Poiyomi 8.2/Poiyomi Pro Geometric Dissolve/695210e43c2f0b5468e8663f66eb7375"
{
	Properties
	{
		...
		[sRGBWarning]_RimMask ("Rim Mask--{reference_properties:[_RimMaskPan, _RimMaskUV, _RimMaskChannel, _RimMaskInvert], condition_showS:_RimStyle==0}", 2D) = "white" { }
 ...
 #define OPTIMIZER_ENABLED 
			...
			void GeometryProgram(triangle VertexOut p[3], inout TriangleStream<VertexOut> triStream)
			{
				...
				{
					geoDissolveAlpha = remapClamped(-1, 1, geoDissolveAlpha, -1 - (0.454 /*_GeoDissolveExpansionTime*/), (0.454 /*_GeoDissolveExpansionTime*/));
					#if defined(PROP_GEODISSOLVEGRADIENT) || !defined(OPTIMIZER_ENABLED)
				...
				{
					geoDissolveAlpha = remapClamped(-1, 1, geoDissolveAlpha, -1 - (0.454 /*_GeoDissolveExpansionTime*/), (0.454 /*_GeoDissolveExpansionTime*/));
					float dissolveGradient = 1 - saturate(inverseLerp3(float4(0,0,0,1).xyz, float4(0,0,1,1).xyz, lerp(center, vertexColorCenter, (0.0 /*_GeoDissolveVertexColor*/))));
					geoDissolveAlpha = (dissolveGradient * .5 - 1) + (geoDissolveAlpha + 1);
				}
				else if (_GeoDissolveGradientType == GEO_DISSOLVE_NOISE)
				{
					...
					float a = 0.0;
					float2 vel = _Time.x * (2.0 /*_GeoDissolveNoiseSpeed*/);
					geoDissolveAlpha = snoise(uvToUse + vel) * .25 + .25;
					a = snoise(uvToUse * float2(cos(_Time.x * (2.0 /*_GeoDissolveNoiseSpeed*/) * 0.15), sin(_Time.x * (2.0 /*_GeoDissolveNoiseSpeed*/) * 0.1)) * 0.1) * 3.1415;
					vel = float2(cos(a), sin(a));
					geoDissolveAlpha += snoise(uvToUse + vel) * .25 + .25;
					geoDissolveAlpha = 1 - (geoDissolveAlpha);
					geoDissolveAlpha = remap(geoDissolveAlpha, (0.646 /*_GeoDissolveNoiseThreshold*/) - (1 - (0.646 /*_GeoDissolveNoiseThreshold*/)), 1, -1 + (saturate(_GeoDissolveAlpha) * 2), 1 - (saturate(-1 * _GeoDissolveAlpha) * 2));
				}
				geoDissolveAlpha = lerp(-1, geoDissolveAlpha, mask);
				geoDissolveAlpha = geoDissolveAlpha / (0.454 /*_GeoDissolveExpansionTime*/);
				...
				if (geoDissolveAlpha < 1.0)
				{
					...
					for (int i = 0; i < 3; i++)
					{
						...
						p[i].localPos.xyz += outDir * geoDissolveAlpha * (0.1 /*_GeoDissolveTravelDistance*/);
					...
					if ((0.0 /*_GeoDissolveShapeType*/) == GEO_DISSOLVE_SHAPE_PYRAMID && geoDissolveAlpha > 0)
					{
						VertexOut o4 = p[0];
						...
						o4.localPos.xyz += outDir * geoDissolveAlpha * (0.1 /*_GeoDissolveTravelDistance*/);
			...
			{
				float4 geoDissolveColor = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, (0.0 /*_GeoDissolveColorThemeIndex*/)), float4(1,1,1,1).a);
				#if defined(PROP_GEODISSOLVETEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				if ((0.0 /*_GeoDissolvePointToPointDebug*/) && _GeoDissolveGradientType == GEO_DISSOLVE_P2P)
				{
					poiFragData.emission = 0;
					float debugValue = inverseLerp3(float4(0,0,0,1).xyz, float4(0,0,1,1).xyz, lerp(poiMesh.localPos, poiMesh.vertexColor.rgb, (0.0 /*_GeoDissolveVertexColor*/)));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = (0.819 /*_ShadowStrength*/) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				shadowStrength = lerp(0, shadowStrength, _OutlineShadowStrength);
				#endif
				#ifdef _LIGHTINGMODE_TEXTURERAMP
				poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + (-0.61 /*_ShadowOffset*/)).rgb, shadowStrength);
			...
			{
				#if defined(PROP_ANISOCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				float4 specMap = POI2D_SAMPLER_PAN(_AnisoColorMap, _MainTex, poiUV(poiMesh.uv[(1.0 /*_AnisoColorMapUV*/)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				...
				#endif
				float spec0 = calculateAnisotropics(lerp(poiMesh.binormal[1], poiMesh.tangent[1], (0.0 /*_Aniso0SwitchDirection*/)), (-0.5 /*_Aniso0Offset*/) +(0.03 /*_Aniso0OffsetMapStrength*/) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.direction, (0.149 /*_Aniso0Power*/) * 1000, (1.0 /*_Aniso0Strength*/), shadowMask);
				float spec1 = calculateAnisotropics(lerp(poiMesh.binormal[1], poiMesh.tangent[1], (0.0 /*_Aniso1SwitchDirection*/)), (0.06 /*_Aniso1Offset*/) +(0.018 /*_Aniso1OffsetMapStrength*/) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.direction, (0.1 /*_Aniso1Power*/) * 1000, (1.0 /*_Aniso1Strength*/), shadowMask);
				...
				float3 finalSpec = saturate(saturate(spec0 * spec0Color) + saturate(spec1 * spec1Color)) * lerp(1, poiFragData.baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, (1.0 /*_AnisoUseLightColor*/));
				float3 baseColor = poiFragData.baseColor;
				poiFragData.baseColor = lerp(poiFragData.baseColor, spec1Color * lerp(1, baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, (1.0 /*_AnisoUseLightColor*/)), (0.0 /*_AnisoReplace*/) * spec1);
				poiFragData.baseColor = lerp(poiFragData.baseColor, spec0Color * lerp(1, baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), poiLight.directColor, (1.0 /*_AnisoUseLightColor*/)), (0.0 /*_AnisoReplace*/) * spec0);
				poiLight.finalLightAdd += max(0, finalSpec * (0.655 /*_AnisoAdd*/));
				if ((1.0 /*_LightingVertexLightingEnabled*/))
				{
					#if defined(VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						float vSpec0 = calculateAnisotropics(lerp(poiMesh.binormal[1], poiMesh.tangent[1], (0.0 /*_Aniso0SwitchDirection*/)), (-0.5 /*_Aniso0Offset*/) +(0.03 /*_Aniso0OffsetMapStrength*/) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.vDirection[index], (0.149 /*_Aniso0Power*/) * 1000, (1.0 /*_Aniso0Strength*/), poiLight.vAttenuationDotNL[index]);
						float vSpec1 = calculateAnisotropics(lerp(poiMesh.binormal[1], poiMesh.tangent[1], (0.0 /*_Aniso1SwitchDirection*/)), (0.06 /*_Aniso1Offset*/) +(0.018 /*_Aniso1OffsetMapStrength*/) * specMap.a, poiMesh.normals[1], poiCam.viewDir, poiLight.vDirection[index], (0.1 /*_Aniso1Power*/) * 1000, (1.0 /*_Aniso1Strength*/), poiLight.vAttenuationDotNL[index]);
						...
						poiLight.finalLightAdd += max(0, saturate(saturate(vSpec0 * spec0Color) + saturate(vSpec1 * spec1Color)) * lerp(1, poiFragData.baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], (1.0 /*_AnisoUseLightColor*/)) * (0.655 /*_AnisoAdd*/));
						poiFragData.baseColor = lerp(poiFragData.baseColor, spec1Color * lerp(1, baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], (1.0 /*_AnisoUseLightColor*/)), (0.0 /*_AnisoReplace*/) * vSpec1);
						poiFragData.baseColor = lerp(poiFragData.baseColor, spec0Color * lerp(1, baseColor, (0.976 /*_AnisoUseBaseColor*/)) * lerp(dot(poiLight.vColor[index], float3(0.299, 0.587, 0.114)), poiLight.vColor[index], (1.0 /*_AnisoUseLightColor*/)), (0.0 /*_AnisoReplace*/) * vSpec0);
					}
					#endif
				}
				return float3(0, 0, 0);
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				matcap.rgb *= (0.36 /*_MatcapIntensity*/);
				...
				blendMatcap(poiLight, poiFragData, poiMods, (0.248 /*_MatcapAdd*/), (0.0 /*_MatcapAddToLight*/), (0.0 /*_MatcapMultiply*/), (0.098 /*_MatcapReplace*/), (0.19 /*_MatcapMixed*/), (0.0 /*_MatcapScreen*/), matcap, matcapMask, (0.0 /*_MatcapEmissionStrength*/), (0.0 /*_MatcapLightMask*/), (0.0 /*_MatcapMaskGlobalMask*/), (2.0 /*_MatcapMaskGlobalMaskBlendType*/));
				#endif
			}
			#endif
