Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/ae228576258f28d4a9c9c3deafaf5b99"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.1.050</color>", Float) = 0
		...
		_LightingIgnoreAmbientColor ("Ignore Ambient Color--{condition_showS:(_LightingMode<=3 || _LightingMode==8)}", Range(0, 1)) = 0
		[Space(15)]
...
#define PROP_TPS_ISSKINNEDMESHRENDERER 0
...
#define PROP_TPS_BUFFEREDDEPTH 0
#define PROP_TPS_BUFFEREDSTRENGTH 0
...
#define PROP_MATCAPMIXED 1
#define PROP_MATCAPADDTOLIGHT 0.048
...
#define PROP_DEPTHRIMBRIGHTNESS 1
...
#define PROPM_START_STYLIZEDSPEC 1
#define PROP_STYLIZEDSPECULAR 1
...
#define PROP_HIGHCOLOR_POWER 0.411
#define PROP_STYLIZEDSPECULARFEATHER 0.289
#define PROP_LAYER1STRENGTH 1
#define PROP_LAYER2SIZE 0.813
#define PROP_STYLIZEDSPECULAR2FEATHER 0.423
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.411)))) * float(1);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0.813)))) * float(0);
				}
				else
				{
					specMask1 += aaBlurStep(specArea, (1.0 - pow(float(0.411), 5)), float(0.289)) * float(1);
					specMask2 += aaBlurStep(specArea, (1.0 - pow(float(0.813), 5)), float(0.423)) * float(0);
				}
				#if defined(PROP_SET_HIGHCOLORMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.1653383,0.1653383,0.1653383,1), float(0)) * lerp(1, poiLight.directColor, float(1)) * specMask * poiLight.occlusion * attenuation * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.1653383,0.1653383,0.1653383,1), float(0)) * lerp(1, poiLight.directColor, float(1)), saturate(specMask * poiLight.occlusion * attenuation * float(1)));
				}
				float3 vSpecMask = 0;
				#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.411)))) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0.813)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = aaBlurStep(specArea, (1.0 - pow(float(0.411), 5)), float(0.289)) * float(1) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, aaBlurStep(specArea, (1.0 - pow(float(0.813), 5)), float(0.423)) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
				}
				vSpecMask *= specularMask;
				if (float(0) == 1)
				{
					poiLight.finalLightAdd += max(0, specularMap * poiThemeColor(poiMods, float4(0.1653383,0.1653383,0.1653383,1), float(0)) * vSpecMask * poiLight.occlusion * float(1));
				}
				else
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, specularMap * poiThemeColor(poiMods, float4(0.1653383,0.1653383,0.1653383,1), float(0)), saturate(vSpecMask * poiLight.occlusion * float(1)));
				}
				#endif
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.2));
