Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/bf4fb4738197190459487807de4b0806"
{
	Properties
	{
...
#define PROP_CLEARCOATBRDF 1
#define PROP_CLEARCOATSTRENGTH 0.598
#define PROP_CLEARCOATSMOOTHNESS 0.609
#define PROP_CLEARCOATREFLECTIONSTRENGTH 0.566
#define PROP_CLEARCOATSPECULARSTRENGTH 0.261
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.291), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			{
				float clearCoatMask = float(0.598);
				float smoothness = float(0.609);
				...
				if (float(0.261) > 0)
				{
					...
					specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * poiLight.occlusion * attenuation * float(0.261);
					#if defined(VERTEXLIGHT_ON) && defined(POI_VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * poiLight.occlusion * poiLight.vAttenuation[index] * float(0.261);
					}
					#endif
				}
				if (float(0.566) > 0)
				{
					...
					float reflStr = reflectionMask * float(0.566);
