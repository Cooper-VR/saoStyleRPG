Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/a82469e50824d124d96aa8e72fe61200"
{
	Properties
	{
...
#define PROP_MATCAPREPLACE 0.438
#define PROP_MATCAPMULTIPLY 0
#define PROP_MATCAPADD 0.33
#define PROP_MATCAPMIXED 0.379
...
#define PROP_RIMSHARPNESS 0.284
...
#define PROP_EMISSIONREPLACE0 1
...
#define PROP_PPEMISSIONMULTIPLIER 6.49
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, float(0.33), float(0), float(0), float(0.438), float(0.379), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.05490196,1.498039,0.05490196,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.05490196,1.498039,0.05490196,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.05490196,1.498039,0.05490196,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rim = 1 - smoothstep(min(float(0.284), rimWidth), rimWidth, viewDotNormal);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#ifdef UNITY_PASS_FORWARDBASE
				poiFragData.emission = max(poiFragData.emission * float(6.49), 0);
