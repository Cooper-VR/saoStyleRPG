...
public class BatchExtractMaterials : EditorWindow
{
    ...
    private string materialsFolder = "Assets/Resources/bmubin/asset_library/allMaterials";
    ...
    private void DrawModelsToProcessList()
    {
        ...
        if (GUILayout.Button("+", GL_WIDTH_25))
        {
            //made by me to add all the files
            string directoryPath = "Assets/Resources/bmubin/asset_library/assets";
            string[] blendFiles = Directory.GetFiles(directoryPath, "*.blend");

            foreach (string filePath in blendFiles)
            {
                ...
                if (prefab != null)
                {
                    modelData.Add(new ExtractData(prefab));
                }
            }
